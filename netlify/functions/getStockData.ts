  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      if (json?.["Error Message"])
        throw new Error(`AlphaVantage error: ${json["Error Message"]}`);
      return json;
    } catch (err) {
      lastErr = err;
...
export const handler: Handler = async (event: HandlerEvent) => {
  const symbol = (event.queryStringParameters?.symbol || "")
    .trim()
    .toUpperCase();
  const wantCandles = (event.queryStringParameters?.candles ?? "1") !== "0";
  ...
  try {
    ...
    const profileUrl = `${baseFinn}/stock/profile2?symbol=${encodeURIComponent(
      symbol
    )}&token=${FINN_KEY}&cb=${cb}`;
    const quoteUrl = `${baseFinn}/quote?symbol=${encodeURIComponent(
      symbol
    )}&token=${FINN_KEY}&cb=${cb}`;
    const dailyFinn = `${baseFinn}/stock/candle?symbol=${encodeURIComponent(
      symbol
    )}&resolution=D&from=${yearAgoSec}&to=${nowSec}&token=${FINN_KEY}&cb=${cb}`;
    ...
    let week52High = 0,
      week52Low = 0,
      volumeToday = 0;
    let candlesSource: "none" | "finnhub" | "fmp" | "alphavantage" | "stooq" =
      "none";
    ...
    if (wantCandles) {
      const tryFinnhub = async () => {
        try {
          ...
          candlesErrors.finnhub = String(e?.message || e);
          throw e;
        }
      };
      ...
      const tryFmp = async () => {
        try {
          ...
          candlesErrors.fmp = String(e?.message || e);
          throw e;
        }
      };
      ...
      const tryAlphaVantage = async () => {
        try {
          ...
          const avDailyUrl = `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${encodeURIComponent(
            symbol
          ...
          candlesErrors.alphavantage = String(e?.message || e);
          throw e;
        }
      };
      ...
      const tryStooq = async () => {
        try {
          ...
          candlesErrors.stooq = String(e?.message || e);
          throw e;
        }
      };
        ...
        if (providerOverride === "fh") await tryFinnhub();
          ...
          try {
            ...
            try {
              ...
              try {
                await tryAlphaVantage();
              } catch {
                await tryStooq().catch(() => {});
              }
            }
          }
        }
      ...
      candlesSource === "none" &&
        (candlesErrors.message ||= "All providers failed");
    }
    ...
    const combined = {
      ...
      _meta: {
        ...
        candlesErrors: wantCandles
          ? Object.keys(candlesErrors).length
            ? candlesErrors
            : undefined
          : undefined,
      },
